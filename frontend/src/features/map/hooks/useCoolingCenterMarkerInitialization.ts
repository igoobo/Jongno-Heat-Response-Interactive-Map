...
interface UseCoolingCenterMarkerInitializationProps {
  map: any;
  layerStates: any;
  ...
  data: any; // The data from fetchCoolingCenterData
}

export const useCoolingCenterMarkerInitialization = ({
  map,
  layerStates,
  markersRef,
  openInfoWindowRef,
  ...
  data,
}: UseCoolingCenterMarkerInitializationProps) => {
  useEffect(() => {
    if (!map || !window.kakao || !data) return;
  ...
  }, [map, layerStates.coolingCenter, data]);
};
